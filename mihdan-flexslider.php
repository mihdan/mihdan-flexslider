...
function mihdan_flexslider_gallery_style( $output ) {
	...
	$output = str_replace( 'gallery ', 'gallery flexslider-container ', $output );
		...
		$js = <<<JS
			jQuery( function( $ ) {			    
			    $('.flexslider-container')
			    	.wrapInner('<div class="flexslider-wrapper"></div>')
			    	//.append('<div class="swiper-pagination"></div>')
			    	//.append('<div class="swiper-button-prev"></div>')
			    	//.append('<div class="swiper-button-next"></div>')			    	
			    	
			  var flexslider = $('.flexslider-container').flexslider({
					animation: 'slide',
					itemWidth: 638,
					selector: '.flexslider-wrapper > .gallery-item'
			  })  
			});
JS;
